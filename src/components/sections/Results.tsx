  ...
  render() {
    ...
    return <Section name="Datos calculados">
      ...
      <Row className="mb-3">
        <Form>
          <Form.Group as={Row}>
            <Form.Label column sm={1} md={2} lg={1} htmlFor="use">
              <a href="https://www.tutiendasolar.es/hps.php" target="_blank">
                HPS:
              </a>
            </Form.Label>
            <Col sm={6} lg={4} className="col-8">
              <Form.Select id="use" size="sm" onChange={(ev) => {
                switch(ev.target.value) {
                  case 'summer':
                    this.props.onChancePeekHours(6.1);
                    break;
                  default:
                    this.props.onChancePeekHours(3.2);
                }
              }}>
            ...
            <Col sm={4} lg={1} className="col-4">
              <Form.Control 
                type="number" 
                size="sm" 
                value={this.props.peekHours}
                onChange={(ev) => {
                  this.props.onChancePeekHours(parseFloat(ev.target.value));
                }}
            ...
            <Col sm={8} lg={3}>
              <Form.Control 
                type="number" 
                size="sm" 
                value={this.props.autonomy} 
                onChange={(ev) => {
                  this.props.onChangeAutonomy(parseInt(ev.target.value));
                }}
                onClick={(ev) => {ev.currentTarget.select();}}
              />
            </Col>
